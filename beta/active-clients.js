(()=>{
  ...
  const COLORS={1:'#58a9ff',2:'#67c26f',3:'#f6a04d',4:'#b06adf'};
  ...
  function sectorForPostalCode(cp){
    const a=getAssign();
    if(a[String(cp)])return Number(a[String(cp)]);
    try{
      if(typeof geo!=='undefined'&&geo?.features?.length&&typeof sectorFor==='function'){
        const sectors=[];
        for(const f of geo.features){
          const cps=(f?.properties?.codesPostaux||[]).map(String);
          if(!cps.includes(String(cp)))continue;
          const s=Number(sectorFor(f)||0);
          if(s>0&&!sectors.includes(s))sectors.push(s);
        }
        if(sectors.includes(1))return 1;
  ...
  function addStyle(){
    if(document.getElementById('activeClientsBetaStyle'))return;
    const s=document.createElement('style');s.id='activeClientsBetaStyle';s.textContent=`
      ...
      .ac-total{background:#0f172a;color:#fff;border-radius:17px;padding:16px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;gap:12px;box-shadow:0 12px 28px rgba(15,23,42,.14)}.ac-total strong{font-size:30px}.ac-total span{font-size:11px;color:#cbd5e1}
      .ac-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:14px}.ac-card{background:#fff;border:1px solid #e2e8f0;border-radius:17px;padding:15px;box-shadow:0 12px 28px rgba(15,23,42,.05);border-top:5px solid var(--ac-color)}.ac-card small{display:block;color:#64748b;font-size:10px}.ac-card strong{display:block;font-size:28px;color:#0f172a;margin:3px 0}.ac-card b{font-size:12px;color:#334155}.ac-sub{font-size:10px;color:#64748b;margin-top:5px}
  ...
  function mount(){
    ...
    addStyle();
    const btn=document.createElement('button');btn.dataset.page='activeclients';btn.id='activeClientsNav';btn.innerHTML='👥 Clients actifs';btn.onclick=switchTo;
    const prospect=nav.querySelector('[data-page="prospection"]');
    if(prospect)nav.insertBefore(btn,prospect);else nav.appendChild(btn);
    const page=document.createElement('section');page.className='rex-page';page.dataset.page='activeclients';page.id='rexActiveClientsPage';page.innerHTML=`
      <div class="ac-headline"><div><h2>Clients actifs par code postal</h2><p>Parc clients actif par CP, avec rattachement automatique au secteur commercial actuel.</p></div></div>
      <div id="acSummary"></div>
      <div class="ac-table-card">
        <div class="ac-toolbar"><div><b>Détail par code postal</b><div style="font-size:10px;color:#64748b">Les totaux suivent les affectations actuelles de la carte.</div></div><input id="acSearch" inputmode="numeric" maxlength="5" placeholder="Ex. 19100"></div>
        ...
        <div class="ac-note">Source : photographie figée du parc clients communiquée précédemment. Les communes verrouillées du Secteur 1 sont prises en compte même lorsque leur code postal n'a pas été affecté manuellement.</div>
      </div>`;
  ...
  function rows(){
    const p=getProfile(),sd=getSectorData();
    return Object.entries(ACTIVE_CLIENTS).map(([cp,clients])=>{
      const sector=sectorForPostalCode(cp);
      const seller=sector?(sd?.[sector]?.seller||`Secteur ${sector}`):'Non affecté';
      return {cp,clients:Number(clients||0),sector,seller};
    }).filter(r=>p?.role!=='seller'||r.sector===Number(p.sector_id));
  }

  function totals(all){
    const t={1:0,2:0,3:0,4:0,unassigned:0,total:0};
    all.forEach(r=>{t.total+=r.clients;if(r.sector>=1&&r.sector<=4)t[r.sector]+=r.clients;else t.unassigned+=r.clients});
    return t;
  }
  ...
  function renderSummary(){
    ...
    if(p?.role==='seller'){
      ...
      box.innerHTML=`<div class="ac-total"><div><b>${esc(p.display_name||'Vendeur')} — Secteur ${s}</b><br><span>${count} code${count>1?'s':''} postal${count>1?'aux':''} renseigné${count>1?'s':''}</span></div><strong>${t.total} clients actifs</strong></div>`;
      return;
    }
    const cards=[1,2,3,4].map(s=>{
      const count=all.filter(r=>r.sector===s).length,seller=sd?.[s]?.seller||'Non attribué';
      return `<div class="ac-card" style="--ac-color:${COLORS[s]}"><small>Secteur ${s}</small><strong>${t[s]}</strong><b>clients actifs</b><div class="ac-sub">${esc(seller)} · ${count} CP renseigné${count>1?'s':''}</div></div>`;
    }).join('');
    box.innerHTML=`<div class="ac-total"><div><b>Agence Corrèze</b><br><span>Parc clients actif par code postal</span></div><strong>${t.total} clients actifs</strong></div><div class="ac-grid">${cards}</div>${t.unassigned?`<div style="margin:-3px 0 13px;font-size:11px;color:#64748b"><b>${t.unassigned}</b> clients actifs sont encore sur des CP non affectés.</div>`:''}`;
  }

  function renderTable(){
    const body=document.getElementById('acBody');if(!body)return;
    const q=(document.getElementById('acSearch')?.value||'').trim();
    const all=rows().filter(r=>!q||r.cp.includes(q)).sort((a,b)=>(a.sector||9)-(b.sector||9)||b.clients-a.clients||a.cp.localeCompare(b.cp));
    body.innerHTML=all.length?all.map(r=>`<tr><td><b>${r.cp}</b></td><td>${r.sector?`<span class="ac-badge" style="background:${COLORS[r.sector]}">S${r.sector}</span>`:'<span class="ac-muted">Non affecté</span>'}</td><td>${esc(r.seller)}</td><td class="ac-num">${r.clients}</td></tr>`).join(''):'<tr><td colspan="4" class="ac-empty">Aucun code postal correspondant.</td></tr>';
  }

  function signature(){
    const a=getAssign(),p=getProfile(),sd=getSectorData();
    let g='';try{g=geo?.features?.length||0}catch(e){}
    return JSON.stringify([a,p?.role,p?.sector_id,sd,g]);
  }
