(()=>{
  ...
  function mount(){
    ...
    style.textContent=`
      ...
      .wf-agency{display:grid;grid-template-columns:repeat(5,minmax(130px,1fr));gap:10px;margin-bottom:14px}.wf-kpi{background:#fff;border:1px solid #d9e2ec;border-radius:12px;padding:12px}.wf-kpi span{display:block;font-size:11px;color:#667085}.wf-kpi strong{font-size:23px;color:#17355f}
  ...
  function sums(list){let cl=0,nc=0,upcross=0,total=0,weighted=0;list.forEach(d=>{const a=Number(d.amount||0);total+=a;weighted+=a*Number(d.probability||0)/100;if(d.client_type==='acquis')cl+=a;else nc+=a;if(d.sale_type==='up_sell'||d.sale_type==='cross_sell')upcross+=a});return{cl,nc,upcross,total,weighted}}
  function render(){
    const all=sums(deals);wfAgency.innerHTML=`<div class="wf-kpi"><span>Prévision CL agence</span><strong>${euro(all.cl)}</strong></div><div class="wf-kpi"><span>Prévision NC agence</span><strong>${euro(all.nc)}</strong></div><div class="wf-kpi"><span>Up / Cross agence</span><strong>${euro(all.upcross)}</strong></div><div class="wf-kpi"><span>Prévision brute agence</span><strong>${euro(all.total)}</strong></div><div class="wf-kpi"><span>Prévision pondérée</span><strong>${euro(all.weighted)}</strong></div>`;
