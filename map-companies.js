(()=>{
  ...
  function ready(){
    try{return !!(mapSvg && geo && typeof sectorFor==='function' && profile)}catch(e){return false}
  }
  ...
  function pathInfo(pathEl){
    try{
      ...
      const f=geo.features[i],s=sectorFor(f);
      if(!canOpenSector(s))return null;
      return {feature:f,sector:s};
    }catch(e){return null}
  }

  async function fetchCommuneCompanies(feature){
    ...
    for(const cp of postalCodes){
      ...
      do{
        ...
        (d.results||[]).forEach(x=>{
          ...
          if(String(sj.code_postal)!==String(cp)||!x.siren)return;
          rows.set(x.siren,{n:x.nom_complet||x.nom_raison_sociale||x.denomination||'Entreprise',s:x.siren,p:x.telephone||x.telephone1||sj.telephone||sj.telephone1||'',a:sj.adresse||[sj.numero_voie,sj.indice_repetition,sj.type_voie,sj.libelle_voie,sj.code_postal,sj.libelle_commune].filter(Boolean).join(' ')});
        });
  ...
  async function openCommune(info){
    const {feature,sector}=info,commune=feature.properties.nom,postalCodes=(feature.properties.codesPostaux||[]).map(String);
    ...
    try{await fetchCommuneCompanies(feature)}catch(e){companyStatus.textContent='Erreur : '+e.message;companyBody.innerHTML='<tr><td colspan="4">Impossible de charger les entreprises.</td></tr>'}
  }
  ...
  function refresh(){
    ...
    paths.forEach((p,i)=>{
      const f=geo.features[i],s=f?sectorFor(f):0,ok=canOpenSector(s);
      p.style.cursor=ok?'pointer':'default';
      p.style.transition='filter .12s, opacity .12s, stroke-width .12s';
  ...
  function mount(){
    if(!ready())return false;
    if(mapSvg.dataset.companyClickMounted==='1'){refresh();return true}
    mapSvg.dataset.companyClickMounted='1';
    mapSvg.addEventListener('click',e=>{
      const p=e.target.closest?.('path.commune');if(!p)return;
      const info=pathInfo(p);if(info)openCommune(info);
    });
    mapSvg.addEventListener('mouseover',e=>{const p=e.target.closest?.('path.commune');if(p?.dataset.mapCompanyEnabled==='1'){p.style.filter='brightness(.90)';p.style.strokeWidth='2'}});
    mapSvg.addEventListener('mouseout',e=>{const p=e.target.closest?.('path.commune');if(p){p.style.filter='';p.style.strokeWidth='0.8'}});
