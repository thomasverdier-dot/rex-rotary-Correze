const APP_CACHE = "rex-correze-app-v7";
...
async function injectModules(response){
  try{
    ...
    const tags=[];
    if(!html.includes('admin-seller.js')) tags.push('<script src="./admin-seller.js?v=7"></script>');
    if(!html.includes('weekly-forecast.js')) tags.push('<script src="./weekly-forecast.js?v=2"></script>');
    if(!html.includes('ui-v3.js')) tags.push('<script src="./ui-v3.js?v=1"></script>');
    if(!html.includes('map-companies.js')) tags.push('<script src="./map-companies.js?v=1"></script>');
