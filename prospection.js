(()=>{
  ...
  function cpOf(x){return String(x?.siege?.code_postal||x?.code_postal||x?.matching_etablissements?.[0]?.code_postal||'')}
  function cityOf(x){return x?.siege?.libelle_commune||x?.siege?.commune||x?.matching_etablissements?.[0]?.libelle_commune||''}
  ...
  function mount(){
    ...
    const page=document.createElement('section');page.className='rex-page';page.dataset.page='prospection';page.id='rexProspectionPage';page.innerHTML=`
      <div class="rex-page-head"><div><h2>Prospection entreprises</h2><p>Recherche par nom, SIREN, commune, effectif et code APE.</p></div></div>
      <div class="pro-card">
        <div class="pro-form">
          <div class="pro-field"><label>Nom de l’entreprise ou SIREN</label><input id="proQ" placeholder="Ex. Dupont BTP ou 123456789"></div>
          <div class="pro-field"><label>Commune ou code postal</label><input id="proPlace" placeholder="Ex. Brive ou 19100"></div>
        ...
        <div class="pro-wrap"><table class="pro-table"><thead><tr><th>Entreprise</th><th>SIREN</th><th>Effectif</th><th>Code APE</th><th>Adresse</th><th>Dirigeant</th><th>Téléphone</th><th>Actions</th></tr></thead><tbody id="proBody"><tr><td colspan="8" class="pro-empty">Aucune recherche lancée.</td></tr></tbody></table></div>
        <div class="pro-note">Les effectifs publics sont des tranches et peuvent dater de N-2/N-3. Les numéros de téléphone ne sont pas systématiquement présents dans les données administratives : le bouton « Trouver le téléphone » lance alors une recherche Google ciblée sur l’entreprise.</div>
      </div>`;
  ...
  async function search(){
    ...
    proStatus.textContent='Recherche en cours…';proBody.innerHTML='<tr><td colspan="8" class="pro-empty">Chargement…</td></tr>';
    try{
      ...
      const r=await fetch(API+'?'+params.toString());if(!r.ok)throw new Error('Erreur API '+r.status);const d=await r.json();let rows=d.results||[];
      ...
      const p=getProfile();if(p?.role==='seller')rows=rows.filter(x=>getOwnCp(cpOf(x)));
      if(phoneOnly)rows=rows.filter(x=>!!phoneOf(x));
      proStatus.textContent=`${rows.length} résultat${rows.length>1?'s':''}${p?.role==='seller'?' sur ton secteur':''}.`;
      ...
      }).join(''):'<tr><td colspan="8" class="pro-empty">Aucune entreprise correspondant aux filtres.</td></tr>';
