(()=>{
  ...
  function mount(){
    ...
    box.innerHTML=`
      <style>
        ...
        #rexSellerAdmin .rex-form{display:none;margin-top:9px}.rex-form.open{display:block}
      ...
      <button class="rex-add" id="rexToggleSeller">＋ Ajouter un vendeur</button>
      <div class="rex-form" id="rexSellerForm">
        ...
        <div class="rex-grid">
          ...
          <select id="rexSellerSector">
            <option value="">Choisir le secteur</option>
            <option value="1">Secteur 1</option><option value="2">Secteur 2</option><option value="3">Secteur 3</option><option value="4">Secteur 4</option>
          </select>
        </div>
        <button class="rex-create" id="rexCreateSeller">Créer et attribuer le secteur</button>
    ...
    const form=box.querySelector('#rexSellerForm'),msg=box.querySelector('#rexSellerMsg'),btn=box.querySelector('#rexCreateSeller');
    box.querySelector('#rexToggleSeller').onclick=()=>form.classList.toggle('open');
    btn.onclick=async()=>{
      ...
      try{
        ...
        msg.className='rex-msg rex-ok';msg.textContent='✓ '+escHtml(display_name)+' créé — Secteur '+sector_id+'.';
  ...
  let tries=0;const timer=setInterval(()=>{tries++;mount();if(document.getElementById('rexSellerAdmin')||tries>60)clearInterval(timer)},250);
